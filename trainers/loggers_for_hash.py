...
def save_state_embeddings(user_embedding, item_embeding, path, filename):
    filename_user='user_embedding_'+filename
    filename_item='item_embedding_'+filename
    torch.save(user_embedding, os.path.join(path, filename_user))
    torch.save(item_embeding, os.path.join(path, filename_item))
...
class BestModelLogger(AbstractBaseLogger):
    ...
    def log(self, *args, **kwargs):
        ...
        if self.best_metric < current_metric:
            ...
            save_state_embeddings(kwargs['user_embedding'],kwargs['item_embedding'],self.checkpoint_path, self.filename)
